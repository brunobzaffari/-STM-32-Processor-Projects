...
int confighoras(int *controle, int *botao_pres);//mais complexa
int sethora(int *hora, int *botao_pres);
int setmins(int *mins, int *botao_pres);
int le_botao(uint8_t a0_in);
//-------------------------------------------------------//
...
{
  /* USER CODE BEGIN 1 */
	int horadset=0, timerset=0, horaset=0, minset=0;
	uint16_t a0_in;
	int botao_pres;
	int kt;
  /* USER CODE END 1 */
  ...
  /* USER CODE BEGIN 2 */
	...
	lcd_goto(0,0);
	printf("Starting at(\n");
 while(horadset==0 ||timerset==0|| horaset==0){
	HAL_ADC_Init(&hadc);
	HAL_ADC_Start(&hadc);
	...
	HAL_ADC_Stop(&hadc);
	// LE OS BOTOES
	botao_pres = le_botao(a0_in);
	     if(horadset==0 && timerset==0 && horaset==0){
		confighorad();
		horadset=sethora(&horad,&botao_pres);}
	else if(horadset==1 && timerset==0 && horaset==0){
		configtimer();
		timerset=setmins(&timer, &botao_pres);}
	else if(horadset==1 && timerset==1 && horaset==0){
		confighoras(&horaset,&botao_pres);
	HAL_Delay(1);
...
//sethora confconfigura hora e horad
//conta tempo pressionado, especifico de 0-23.
int le_botao(int a0_in){
	//OS VALORES APROXIMADOS FORAM 0, 572, 1500, 2470, 3775
	if (a0_in >= 4095) return = PRES_NONE;// NENHUM/NAO APERTA
	else if(a0_in <=500) return = PRES_DIR;// DIREITA
	else if((a0_in >=500)&&(a0_in <=700)) return = PRES_INC;// INC
	else if((a0_in >=1500)&&(a0_in <=2000))  return = PRES_DEC;// DEC
	else if((a0_in >=2400)&&(a0_in <=2700))  return = PRES_ESQ;// ESQUERDA
	else if((a0_in >= 3600)&&(a0_in <4002))  return = PRES_ENTER;// ENTER
	else  return = PRES_NONE;  // DEFALT NENHUM/NAO APERTA
 }
...
int confighoras(int *controle, int *botao_pres){
	lcd_goto(0,0);
